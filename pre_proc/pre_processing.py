...
def load_bypath_data(args, nlp):
    ...
    with open(data_file, 'r') as file:
        ...
        for line in pbar:
            ...
            # tp_list.append((person[0], jobs, education, skills))
            tp_list.append((person[0], jobs))
            pbar.update(1)
            id_counter += 1
...
def build_dataset(args):
    ...
    # data = [person for person in tp_list if (len(person[1]) > 0) and (len(person[2]) > 0) and (len(person[3]) > 0)]
    data = [person for person in tp_list if (len(person[1]) > 0)]
    data_test = []
    data_train = []
    ...
    return {"train_data": data_train, "test_data": data_test}
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output", type=str, default="pkl/people_industry_indexed.pkl")
