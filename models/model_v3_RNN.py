...
def main(args):
    xp_title = "V3 bs" + str(args.batch_size) + " dpo" + str(args.dpo) + " lr" + str(args.lr)
    ...
    ## profile RNN
    rnn = CareerEncoderLSTM(hidden_size * 2, hidden_size * 2, 1, args.dpo, args.bidirectional)
    rnn = rnn.cuda()
    ...
    dictionary = main_for_one_split(args, enc_job, dec_job, rnn, mlp, dataset_train, dataset_valid, hidden_size*2)
...
def main_for_one_epoch(epoch, encoder, decoder, rnn, mlp, criterion,
    ...
    train_loss = train(encoder, decoder, rnn, mlp, dataloader_train, criterion, optim, args.MAX_SEQ_LENGTH, hidden_size, epoch)
